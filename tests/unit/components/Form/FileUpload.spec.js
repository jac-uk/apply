    ...
    describe('upload()', () => {
      ...
      describe('with file', () => {
        ...
        describe('put() returned a valid response', () => {
          ...
          it('sets `fileName` to predefined name with original extension', async () => {
            expect.assertions(1);
            const computedFileName = `${mockProps.name}.${mockFile.name.split('.')[1]}`;

            await wrapper.vm.upload(mockFile);

            expect(wrapper.emitted().input).toEqual([[computedFileName]]);
          });
